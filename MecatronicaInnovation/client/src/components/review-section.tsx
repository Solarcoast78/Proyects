...
export default function ReviewSection({ challengeType, challengeTitle }: ReviewSectionProps) {
  ...
  const { data: reviews = [], isLoading } = useQuery({
    queryKey: ['/api/reviews', challengeType],
  ...
  const submitReviewMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['/api/reviews', challengeType] });
  ...
  return (
    <div className="max-w-4xl mx-auto mt-16 space-y-8">
      ...
      <div className="text-center">
        <h2 className="text-3xl font-bold text-gray-900 mb-4">Reviews y Opiniones</h2>
